...
<head>
    ...
    <script type="text/javascript">
        ...
        const boardDetail = (b_no) => {
            location.href="/board/boardDetail?b_no="+b_no; /*BoardController - boardDetail로 이동*/
        }
        ...
        const  selectLocalImage = (event) =>  {
            ...
            // <input type=file/>
            const fileInput = document.createElement('input');//<input> - DOM API의 createElement를 사용하여 태그 생성하는 코드(react에서도 필요한 코드)
            ...
            // 사람이 눌러야 할 이벤트를 코드에서 대신 클릭해 줌
            fileInput.click();

            fileInput.addEventListener("change", function () {  // change 이벤트로 input 값이 바뀌면 실행
                const formData = new FormData();
                const file = fileInput.files[0]; // 배열로 여러 개 파일 처리 가능함
                formData.append('image', file);
                // 글쓰기(insert)가 처리되기 전에 먼저 미리보기 처리를 해야 한다
                $.ajax({//<form method=post enctype=multipart/form-data/> 이럴경우 req.getParameter사용이 불가함 -> cos.jar
                    type: 'post',
                    enctype: 'multipart/form-data',
                    url: '/board/imageUpload', // 백엔드에서 요청을 처리한 후 파일명을 반환받기 : @RestController 사용함
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function (response) {
                        console.log('avatar.png'+response); //avartar.png
                        // imageUpload 요청을 처리하면 리턴값으로 아바타.png가 온다
                        // demo서버에서 pds폴더에 업로드(57번 url이 요청) 되어있는 이미지 정보를 읽어온다
                        // 서버에 있는 이미지를 읽어오는거니까 get방식도 괜찮아
                        // request.getParatmeter("imageName"), @RequestParam("imageName") String imageName
                        const url = "/board/imageGet?imageName="+response;// + style=~ 붙음
                        // range : 선택된 이미지가 들어갈 위치 정보를 쥔다
        ...
        <table class="table table-hover">
            ...
            <%
                for(int i=0; i<size; i++) {
                    if(i==size) break; // NullPointerException 방어
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="f_board" method="post" enctype="multipart/form-data" action="./boardInsert">
                    <%--hidden : 화면에는 보이지 않지만 몰래 백엔드에 전달할 값이 있을 때--%>
                    <input type="hidden" id="b_content" name="b_content"><%--hidden이 들어가지 않으면 망함!--%>
                    ...
                    <script>
                        ...
                        console.log(html);
                        // 에디터에 변경(택스트, 이미지)이 발생하면 그 때 이벤트처리
                        // 새 글 쓰기에 필요한 api와 수정하기에 필요한 api가 다르다
                        quill.on('text-change', (delta, oldDelta, source) => {
                            console.log('글자가 입력될때 마다 호출');
                            console.log(quill.root.innerHTML);
                            // 에디터에 들어온 내용이 div에 삽입되어야 한다
                            // b_content 안에 들어갈 정보이다 - pmap.put("b_content", "<p><br>머시기</p>")
                            // innerHTML을 사용하면 태그가 포함된 문자열이나 이미지 태그 정보가 포함됨 <img src=아바타.png>
                            document.querySelector("#editor").value = quill.root.innerHTML;
                            // insert here : textarea 대신에 사용자가 작성한 내용을 서버에 전송해야 함
                        ...
                        quill.getModule('toolbar').addHandler('image',  () => {
                            ...
                            // 1) 플젝이 바라보는 pds폴더에 선택한 이미지를 업로드 한다
                            // 2) <img src="이미지경로와 이름">
                            selectLocalImage();
                        })
                    </script>
